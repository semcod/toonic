...
class ContextAccumulator:
        ...
        with self._lock:
            ...
            if chunk.is_delta and chunk.source_id in self._chunks:
                ...
                if len(self._history[chunk.source_id]) > 10:
                    self._history[chunk.source_id] = self._history[chunk.source_id][
                        -10:
                    ]
        ...
        with self._lock:
            for cat, chunks in self._cat_chunks.items():
                ...
                for chunk in sorted_chunks:
                    est = chunk.token_estimate or (
                        len(chunk.toon_spec.split()) * 4 // 3 if chunk.toon_spec else 0
                    )
    ...
    def _enforce_budget(self, category: str) -> None:
        ...
        total = sum(
            c.token_estimate
            or (len(c.toon_spec.split()) * 4 // 3 if c.toon_spec else 0)
            for c in chunks
        )
        ...
        while total > budget and chunks:
            ...
            est = evicted.token_estimate or (
                len(evicted.toon_spec.split()) * 4 // 3 if evicted.toon_spec else 0
            )
            total -= est
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            stats = {
                ...
                "per_category": {
                    cat: {"sources": cat_counts[cat], "tokens": cat_tokens[cat]}
    ...
    def clear(self, category: Optional[str] = None) -> None:
        with self._lock:
            if category:
                ...
                to_remove = [
                    sid
                    for sid, c in self._chunks.items()
                    if (
                        c.category.value
                        if isinstance(c.category, SourceCategory)
                        else c.category
                    )
                    == category
                ]
