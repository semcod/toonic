...
class EventCondition:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.min_size_pct > 0:
            d["min_size_pct"] = self.min_size_pct
        if self.max_size_pct < 100:
            d["max_size_pct"] = self.max_size_pct
        if self.min_speed > 0:
            d["min_speed"] = self.min_speed
        if self.direction:
    ...
    def from_dict(cls, d: Dict[str, Any]) -> EventCondition:
        return cls(
            **{k: v for k, v in d.items() if hasattr(cls, k) and k != "params"},
            params=d.get("params", {}),
        )
...
class FallbackConfig:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.periodic_s > 0:
            d["periodic_s"] = self.periodic_s
        if self.on_silence_s > 0:
            d["on_silence_s"] = self.on_silence_s
...
class TriggerRule:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.event_logic != "any":
            d["event_logic"] = self.event_logic
        fb = self.fallback.to_dict()
        if fb:
            d["fallback"] = fb
        if self.cooldown_s != 5.0:
            d["cooldown_s"] = self.cooldown_s
        if self.goal:
            d["goal"] = self.goal
        if self.actions != ["send_to_llm"]:
    ...
    def from_dict(cls, d: Dict[str, Any]) -> TriggerRule:
        events = [
            EventCondition.from_dict(e) if isinstance(e, dict) else e
            for e in d.get("events", [])
        ]
        fallback = (
            FallbackConfig.from_dict(d["fallback"])
...
class TriggerConfig:
    ...
    def from_dict(cls, d: Dict[str, Any]) -> TriggerConfig:
        triggers = [
            TriggerRule.from_dict(t) if isinstance(t, dict) else t
...
def dump_triggers(config: TriggerConfig) -> str:
    ...
    return yaml.dump(
        config.to_dict(), default_flow_style=False, sort_keys=False, allow_unicode=True
    )
