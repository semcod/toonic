...
def main():
    ...
    chunks_to_add = [
        (
            "info-1",
            SourceCategory.LOGS,
            "2025-01-15 INFO [api] GET /health → 200 OK (12ms)",
        ...
        (
            "info-2",
            SourceCategory.LOGS,
            "2025-01-15 INFO [api] GET /users → 200 OK (45ms)",
            ContentType.LOG_ENTRIES,
            0.3,
        ),
        (
            "info-3",
            SourceCategory.LOGS,
            "2025-01-15 INFO [cache] Hit rate: 94%",
            ContentType.LOG_ENTRIES,
            0.3,
        ),
        (
            "warn-1",
            SourceCategory.LOGS,
            "2025-01-15 WARNING [db] Slow query: 1200ms SELECT * FROM orders",
            ContentType.LOG_ENTRIES,
            0.5,
        ),
        (
            "error-1",
            SourceCategory.LOGS,
            "2025-01-15 ERROR [db] Connection pool exhausted — 0/100 available",
            ContentType.LOG_ENTRIES,
            0.8,
        ),
        (
            "critical-1",
            SourceCategory.LOGS,
            "2025-01-15 CRITICAL [db] Replication lag 15s — threshold 5s",
            ContentType.LOG_ENTRIES,
            1.0,
        ),
        (
            "code-1",
            SourceCategory.CODE,
            "M app.py | c UserService | m get_user(id) | m delete_user(id)",
            ContentType.TOON_SPEC,
            0.5,
        ),
        (
            "info-4",
            SourceCategory.LOGS,
            "2025-01-15 INFO [api] POST /login → 200 OK (230ms)",
            ContentType.LOG_ENTRIES,
            0.3,
        ),
        (
            "info-5",
            SourceCategory.LOGS,
            "2025-01-15 INFO [worker] Processed 42 jobs in batch",
            ContentType.LOG_ENTRIES,
            0.3,
        ),
        (
            "error-2",
            SourceCategory.LOGS,
            "2025-01-15 ERROR [auth] Failed login for user admin from 10.0.0.1",
            ContentType.LOG_ENTRIES,
            0.8,
        ),
    ]
    ...
    for c in chunks:
        ...
        pri_label = {
            ...
            0.3: "LOW",
            0.1: "TRACE",
        }.get(c.priority, f"{c.priority:.1f}")
