...
class TestGenericPrompt:
    def test_includes_goal(self):
        ...
        result = builder.build(
            ...
            chunks=[
                ContextChunk(
                    source_id="test", category=SourceCategory.CODE, toon_spec="hello"
                )
            ],
            images=[],
        )
...
class TestCCTVEventPrompt:
    ...
    def test_no_detections(self):
        ...
        result = builder.build(
            ...
            chunks=[
                ContextChunk(
                    source_id="cam1", category=SourceCategory.VIDEO, toon_spec=""
                )
            ],
            images=["frame1"],
        )
...
class TestSelectPromptBuilder:
    def test_cctv(self):
        builder = select_prompt_builder(
            "CCTV security monitoring", {SourceCategory.VIDEO}
        )
...
class TestAccumulatorPriority:
    def test_high_priority_survives_eviction(self):
        ...
        acc = ContextAccumulator(
            config=BudgetConfig(total_tokens=500, allocations={"logs": 0.75})
        )
    ...
    def test_get_stats_shows_utilization(self):
        acc = ContextAccumulator(
            config=BudgetConfig(total_tokens=10000, allocations={"code": 0.5})
        )
        acc.update(
            ContextChunk(
                source_id="test",
                category=SourceCategory.CODE,
                toon_spec="hello world",
            )
        )
        stats = acc.get_stats()
    ...
    def test_clear_by_category(self):
        acc = ContextAccumulator(max_tokens=10000)
        acc.update(
            ContextChunk(source_id="s1", category=SourceCategory.CODE, toon_spec="code")
        )
        acc.update(
            ContextChunk(source_id="s2", category=SourceCategory.LOGS, toon_spec="log")
        )
        acc.clear("code")
        stats = acc.get_stats()
...
class TestContentTypeModel:
    def test_default_values(self):
        chunk = ContextChunk(
            source_id="test", category=SourceCategory.CODE, toon_spec="x"
        )
